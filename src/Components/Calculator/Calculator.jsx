    ...
    <div className="py-[2rem]  md:px-[33rem]">
      ...
      >
        <div className="text-end  pt-[2rem] pr-[1rem]">
          <h2
            className="text-[#fff] text-end font-bold text-5xl"
            style={styles}
          >
            {!showResult ? (firstInput > 0 ? firstInput : 0) : result}
          </h2>
        </div>
      </div>
      <div
      ...
      >
        <div className="grid grid-cols-4 gap-4   p-[1rem] ">
          {keys.map((key) => (
            <button
              ...
              value={key.key}
              className="h-[4rem] w-w-ful text-3xl font-bold bg-[#E9E2DA]   hover:bg-[#fff] rounded-md drop-shadow-xl border-b-4 border-[#B6AB9D] ]"
              style={
                key.key === "DEL"
                  ? {
                  ...
                  : { backgroundColor: "#E9E2DA", hover: "#fff" }
              }
            >
              {key.key}
            </button>
        ...
        <div className="grid grid-cols-2 gap-4 px-[1rem] pb-[2rem] ">
          <button
            onClick={resetInput}
            className="h-[4rem] w-full text-2xl text-[#fff] font-bold bg-[#637197] hover:bg-[#c5d0f0] rounded-md drop-shadow-xl border-b-4 border-[#575E75]  "
          >
            RESET
          </button>
          <button
            value="7"
            className="h-[4rem] w-full text-3xl text-[#fff] font-bold bg-[#D13F30] hover:bg-[#f2948a] rounded-md drop-shadow-xl border-b-4 border-[#832720] "
            onClick={showUserResult}
          >
            =
          </button>
        </div>
